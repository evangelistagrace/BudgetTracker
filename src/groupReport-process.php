...
$query2 = pg_query("SELECT * FROM groupbudgets WHERE EXTRACT(MONTH FROM budgetdate) = $month AND EXTRACT(YEAR FROM budgetdate) = $year AND groupingid = $groupingid ORDER BY budgetid ASC ");
// corresponding budget/expense category color code
$query3 = pg_query("SELECT * FROM groupcolors WHERE groupingid = $groupingid ");
// total expenses
$query4 = pg_query("SELECT SUM(expenseamount) AS totalexpenses FROM groupexpenses WHERE EXTRACT(MONTH FROM expensedate) = $month AND EXTRACT(YEAR FROM expensedate) = $year groupingid = $groupingid ");
$totalExpenses = pg_fetch_array($query4);
$totalexpense = $totalExpenses['totalexpenses'];
// budget usage percentages
$query5 = pg_query("SELECT * FROM groupbudgets WHERE EXTRACT(MONTH FROM budgetdate) = $month AND EXTRACT(YEAR FROM budgetdate) = $year AND groupingid = $groupingid ORDER BY budgetid ASC");
while($result5 = pg_fetch_array($query5)){
    // expenses total by budget/category
    $query = pg_query("SELECT budgetid, SUM (expenseamount) as total FROM groupexpenses WHERE EXTRACT(MONTH FROM expensedate) = $month AND EXTRACT(YEAR FROM expensedate) = $year AND groupingid = $groupingid GROUP BY budgetid ORDER BY budgetid ASC");
    ...
    while($expense = pg_fetch_array($query)){
        if($result5['budgetid'] == $expense['budgetid']){
            ...
            $color = pg_fetch_array(pg_query("SELECT * FROM groupcolors WHERE colorname = '".$result5['budgetcolor']."' "));
...
$query5 = pg_query("SELECT expensedate, budgetid, SUM(expenseamount) AS totalexpenses, EXTRACT(DAY FROM expensedate) AS expenseday FROM groupexpenses WHERE EXTRACT(MONTH FROM expensedate) = $month AND EXTRACT(YEAR FROM expensedate) = $year AND groupingid = $groupingid GROUP BY expensedate, budgetid ORDER BY expensedate ASC ");

$query8 = pg_query("SELECT expensedate, SUM(expenseamount) AS totalexpenses, EXTRACT(DAY FROM expensedate) AS expenseday FROM groupexpenses WHERE EXTRACT(MONTH FROM expensedate) = $month AND EXTRACT(YEAR FROM expensedate) = $year AND groupingid = $groupingid GROUP BY expensedate ORDER BY expensedate ASC ");
...
$query6 = pg_query("SELECT * FROM groupbudgets WHERE EXTRACT(MONTH FROM budgetdate) = $month AND EXTRACT(YEAR FROM budgetdate) = $year AND groupingid = $groupingid ORDER BY budgetname ASC");
